...
def binarize_new(image):
    ...
    binary = np.array([np.clip(sub_arr, 10, 50) for sub_arr in image], dtype=object)  
    #! working!
    binary  = img_as_ubyte(np.clip(2 * image, 10, 50))
    #
    #
    # old
    thresh = ski.filters.threshold_otsu(image)
    binary =cv.Canny(image, 1, thresh)
#    binary = image > thresh
#    binary = np.clip(image * 5, 0, 10)
