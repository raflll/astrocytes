...
def extract_features(binarized_img, skeleton_pruned, image_name=None):
    ...
    # Extract connected components for Otsu and Triangle pruned skeletons
    num_components, labels = cv2.connectedComponents(binarized_img)

    features = []
    fractal_dims = []

    # Iterate over the connected components in Otsu image
    for label in range(1, num_components):  
        ...
        if num_projs2 > 0:
            fractal_dim = calculate_fractal_dimension(cropped_mask)
        else:
            fractal_dim = 0
...
def analyze_projections(skeleton_component):
    ...
    labeled_path_img = pcv.morphology.segment_path_length(segmented_img=segments, objects=projection_objs)
...
def calculate_fractal_dimension(binarized_img_cropped):
    ...
    except Exception as e: # doesnt get called
